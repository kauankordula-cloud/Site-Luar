(() => {
  ...
  function adicionar(id) {
    ...
    const item = carrinho.find(linha => linha.id === id);
  ...
  document.addEventListener("click", evento => {
    ...
    if (adicionarBotao) {
      adicionar(adicionarBotao.dataset.addOrder);
      return;
    }
    ...
    if (!acaoBotao) return;
    const linha = acaoBotao.closest("[data-order-id]");
    const item = carrinho.find(produto => produto.id === linha?.dataset.orderId);
    if (!item) return;
